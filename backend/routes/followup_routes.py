...
@router.post("/{followup_id}/complete", response_model=followup_schema.FollowUp)
def complete_followup(followup_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    return followup_service.complete_followup(db, followup_id=followup_id)
